               ...
               double *rnorm)
{
    ...
    //---->
    //---------------------------------------------------------------------
    // #pragma omp parallel for default(shared) reduction(+:d, sum, rho)
    ...
    for (cgit = 1; cgit <= cgitmax; cgit++)
    {
        ...
        // #pragma omp parallel private(sum, d, alpha, rho){
        d = 0.0;
        #pragma omp parallel for schedule(dynamic) reduction(+:sum, d)
        ...
        //---------------------------------------------------------------------
        // d = 0.0;
        // // #pragma omp parallel for reduction(+:d) lastprivate(d)
        ...
        // Now, obtain the norm of r: First, sum squares of r elements locally...
        //---------------------------------------------------------------------
        // #pragma omp parallel for reduction(+:rho) lastprivate(rho)
        // #pragma omp parallel for reduction(+:rho)
        // for (int j = 0; j < lastcol - firstcol + 1; j++)
        // {
        //     //TODO atomic
    ...
    #pragma omp parallel for schedule(runtime) reduction(+:d, sum)
    for (int j = 0; j < lastrow - firstrow + 1; j++)
    {
        d = 0.0;
        for (int k = rowstr[j]; k < rowstr[j + 1]; k++)
