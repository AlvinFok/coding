...
__global__ void mandelKernel(float x0, float y0, float dx, float dy, int width, int* img, int maxIterations, int pitch) {
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    mandelKernel<<<numBlock, threadPerBlock >>>(lowerX, lowerY, stepX, stepY, resX, d_img, maxIterations, pitch);
